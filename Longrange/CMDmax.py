...
UART_ADDR = 0
TEMP_C    = 25.0
TARGET_M  = 2.1  # ← 1.2, 1.3, 1.4, 1.5, 2.0 — whatever you want to test
N_LOOPS   = 30
# ─────────────────────────────────────────────────────────────────────────────
...
def write_thresholds_exact(ser, thr_level=0x30):
    ...
    time.sleep(0.100)
    print(f"  CMD16 bulk write: T1=8000µs level=0x{thr_level:02X}({thr_level}/255)")
...
def main():
    ...
    fname = f'cmdMAX8000us__2_{TARGET_M}m.png'
